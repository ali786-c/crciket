  ...
  for ($i = 0; $i -lt $Retries; $i++) {
    try {
      if($OutFile -ne '') {
        ...
        curl.exe -L -k -s -o $OutFile $Url
        if ((Get-Item $OutFile).Length -lt 1000) {
          throw "Download failed or file too small"
        }
      } else {
        $result = curl.exe -L -k -s $Url
      ...
      if ($i -eq ($Retries - 1)) {
        if($FallbackUrl) {
          try {
            if($OutFile -ne '') {
              if (Test-Path $OutFile) { Remove-Item $OutFile -Force }
              curl.exe -L -k -s -o $OutFile $FallbackUrl
              if ((Get-Item $OutFile).Length -lt 1000) {
                throw "Fallback download failed"
              }
            } else {
              $result = curl.exe -L -k -s $FallbackUrl
              return $result
            }
          } catch {
